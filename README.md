...
说明：
- 自动检查并编译 `build/qwen_minimal`（若缺失）。
- 自动检查并转换 qmini 权重（若缺失）。
- 直接进入多轮对话模式（输入 `/exit` 退出，`/clear` 清空历史）。

可通过环境变量覆盖路径：

PYTHON_BIN=/root/miniconda/bin/python \
MODEL_DIR=/root/QweninCuda/ms_models/Qwen/Qwen3___5-4B \
WEIGHTS_PATH=/root/QweninCuda/weights/qwen3_5_4b_strict_seq512.qmini \
./run_demo.sh
